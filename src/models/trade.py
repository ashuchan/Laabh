"""Paper trades — executed buy/sell orders against a portfolio."""
from __future__ import annotations

import uuid
from datetime import datetime

from sqlalchemy import DateTime, ForeignKey, Integer, Numeric, Text, func
from sqlalchemy.dialects.postgresql import UUID
from sqlalchemy.orm import Mapped, mapped_column

from src.db import Base
from src.models._types import ORDER_TYPE, TRADE_STATUS, TRADE_TYPE


class Trade(Base):
    """An executed (or cancelled) paper-trade order."""

    __tablename__ = "trades"

    id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), primary_key=True, server_default=func.uuid_generate_v4()
    )
    portfolio_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("portfolios.id"), nullable=False
    )
    instrument_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("instruments.id"), nullable=False
    )
    signal_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("signals.id")
    )

    trade_type: Mapped[str] = mapped_column(TRADE_TYPE, nullable=False)
    order_type: Mapped[str] = mapped_column(ORDER_TYPE, nullable=False, server_default="MARKET")
    quantity: Mapped[int] = mapped_column(Integer, nullable=False)
    price: Mapped[float] = mapped_column(Numeric(12, 2), nullable=False)
    limit_price: Mapped[float | None] = mapped_column(Numeric(12, 2))
    trigger_price: Mapped[float | None] = mapped_column(Numeric(12, 2))

    brokerage: Mapped[float] = mapped_column(Numeric(10, 2), server_default="0")
    stt: Mapped[float] = mapped_column(Numeric(10, 2), server_default="0")
    total_cost: Mapped[float | None] = mapped_column(Numeric(12, 2))

    status: Mapped[str] = mapped_column(TRADE_STATUS, server_default="open")

    closing_trade_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("trades.id")
    )
    pnl: Mapped[float | None] = mapped_column(Numeric(12, 2))
    pnl_pct: Mapped[float | None] = mapped_column(Numeric(8, 4))
    holding_days: Mapped[int | None] = mapped_column(Integer)

    entry_reason: Mapped[str | None] = mapped_column(Text)
    exit_reason: Mapped[str | None] = mapped_column(Text)
    lessons_learned: Mapped[str | None] = mapped_column(Text)

    executed_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    closed_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
